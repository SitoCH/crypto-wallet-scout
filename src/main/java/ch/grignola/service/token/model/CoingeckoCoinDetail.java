package ch.grignola.service.token.model;

import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;

import java.util.List;

@JsonInclude(JsonInclude.Include.NON_NULL)
class Ath {

    @JsonProperty("aed")
    public float aed;
    @JsonProperty("ars")
    public float ars;
    @JsonProperty("aud")
    public float aud;
    @JsonProperty("bch")
    public float bch;
    @JsonProperty("bdt")
    public float bdt;
    @JsonProperty("bhd")
    public float bhd;
    @JsonProperty("bmd")
    public float bmd;
    @JsonProperty("bnb")
    public float bnb;
    @JsonProperty("brl")
    public float brl;
    @JsonProperty("btc")
    public float btc;
    @JsonProperty("cad")
    public float cad;
    @JsonProperty("chf")
    public float chf;
    @JsonProperty("clp")
    public float clp;
    @JsonProperty("cny")
    public float cny;
    @JsonProperty("czk")
    public float czk;
    @JsonProperty("dkk")
    public float dkk;
    @JsonProperty("dot")
    public float dot;
    @JsonProperty("eos")
    public float eos;
    @JsonProperty("eth")
    public float eth;
    @JsonProperty("eur")
    public float eur;
    @JsonProperty("gbp")
    public float gbp;
    @JsonProperty("hkd")
    public float hkd;
    @JsonProperty("huf")
    public float huf;
    @JsonProperty("idr")
    public float idr;
    @JsonProperty("ils")
    public float ils;
    @JsonProperty("inr")
    public float inr;
    @JsonProperty("jpy")
    public float jpy;
    @JsonProperty("krw")
    public float krw;
    @JsonProperty("kwd")
    public float kwd;
    @JsonProperty("lkr")
    public float lkr;
    @JsonProperty("ltc")
    public float ltc;
    @JsonProperty("mmk")
    public float mmk;
    @JsonProperty("mxn")
    public float mxn;
    @JsonProperty("myr")
    public float myr;
    @JsonProperty("ngn")
    public float ngn;
    @JsonProperty("nok")
    public float nok;
    @JsonProperty("nzd")
    public float nzd;
    @JsonProperty("php")
    public float php;
    @JsonProperty("pkr")
    public float pkr;
    @JsonProperty("pln")
    public float pln;
    @JsonProperty("rub")
    public float rub;
    @JsonProperty("sar")
    public float sar;
    @JsonProperty("sek")
    public float sek;
    @JsonProperty("sgd")
    public float sgd;
    @JsonProperty("thb")
    public float thb;
    @JsonProperty("try")
    public float _try;
    @JsonProperty("twd")
    public float twd;
    @JsonProperty("uah")
    public float uah;
    @JsonProperty("usd")
    public float usd;
    @JsonProperty("vef")
    public float vef;
    @JsonProperty("vnd")
    public long vnd;
    @JsonProperty("xag")
    public float xag;
    @JsonProperty("xau")
    public float xau;
    @JsonProperty("xdr")
    public float xdr;
    @JsonProperty("xlm")
    public float xlm;
    @JsonProperty("xrp")
    public float xrp;
    @JsonProperty("yfi")
    public float yfi;
    @JsonProperty("zar")
    public float zar;
    @JsonProperty("bits")
    public float bits;
    @JsonProperty("link")
    public float link;
    @JsonProperty("sats")
    public float sats;

}

@JsonInclude(JsonInclude.Include.NON_NULL)
class AthChangePercentage {

    @JsonProperty("aed")
    public float aed;
    @JsonProperty("ars")
    public float ars;
    @JsonProperty("aud")
    public float aud;
    @JsonProperty("bch")
    public float bch;
    @JsonProperty("bdt")
    public float bdt;
    @JsonProperty("bhd")
    public float bhd;
    @JsonProperty("bmd")
    public float bmd;
    @JsonProperty("bnb")
    public float bnb;
    @JsonProperty("brl")
    public float brl;
    @JsonProperty("btc")
    public float btc;
    @JsonProperty("cad")
    public float cad;
    @JsonProperty("chf")
    public float chf;
    @JsonProperty("clp")
    public float clp;
    @JsonProperty("cny")
    public float cny;
    @JsonProperty("czk")
    public float czk;
    @JsonProperty("dkk")
    public float dkk;
    @JsonProperty("dot")
    public float dot;
    @JsonProperty("eos")
    public float eos;
    @JsonProperty("eth")
    public float eth;
    @JsonProperty("eur")
    public float eur;
    @JsonProperty("gbp")
    public float gbp;
    @JsonProperty("hkd")
    public float hkd;
    @JsonProperty("huf")
    public float huf;
    @JsonProperty("idr")
    public float idr;
    @JsonProperty("ils")
    public float ils;
    @JsonProperty("inr")
    public float inr;
    @JsonProperty("jpy")
    public float jpy;
    @JsonProperty("krw")
    public float krw;
    @JsonProperty("kwd")
    public float kwd;
    @JsonProperty("lkr")
    public float lkr;
    @JsonProperty("ltc")
    public float ltc;
    @JsonProperty("mmk")
    public float mmk;
    @JsonProperty("mxn")
    public float mxn;
    @JsonProperty("myr")
    public float myr;
    @JsonProperty("ngn")
    public float ngn;
    @JsonProperty("nok")
    public float nok;
    @JsonProperty("nzd")
    public float nzd;
    @JsonProperty("php")
    public float php;
    @JsonProperty("pkr")
    public float pkr;
    @JsonProperty("pln")
    public float pln;
    @JsonProperty("rub")
    public float rub;
    @JsonProperty("sar")
    public float sar;
    @JsonProperty("sek")
    public float sek;
    @JsonProperty("sgd")
    public float sgd;
    @JsonProperty("thb")
    public float thb;
    @JsonProperty("try")
    public float _try;
    @JsonProperty("twd")
    public float twd;
    @JsonProperty("uah")
    public float uah;
    @JsonProperty("usd")
    public float usd;
    @JsonProperty("vef")
    public float vef;
    @JsonProperty("vnd")
    public float vnd;
    @JsonProperty("xag")
    public float xag;
    @JsonProperty("xau")
    public float xau;
    @JsonProperty("xdr")
    public float xdr;
    @JsonProperty("xlm")
    public float xlm;
    @JsonProperty("xrp")
    public float xrp;
    @JsonProperty("yfi")
    public float yfi;
    @JsonProperty("zar")
    public float zar;
    @JsonProperty("bits")
    public float bits;
    @JsonProperty("link")
    public float link;
    @JsonProperty("sats")
    public float sats;

}

@JsonInclude(JsonInclude.Include.NON_NULL)
class AthDate {

    @JsonProperty("aed")
    public String aed;
    @JsonProperty("ars")
    public String ars;
    @JsonProperty("aud")
    public String aud;
    @JsonProperty("bch")
    public String bch;
    @JsonProperty("bdt")
    public String bdt;
    @JsonProperty("bhd")
    public String bhd;
    @JsonProperty("bmd")
    public String bmd;
    @JsonProperty("bnb")
    public String bnb;
    @JsonProperty("brl")
    public String brl;
    @JsonProperty("btc")
    public String btc;
    @JsonProperty("cad")
    public String cad;
    @JsonProperty("chf")
    public String chf;
    @JsonProperty("clp")
    public String clp;
    @JsonProperty("cny")
    public String cny;
    @JsonProperty("czk")
    public String czk;
    @JsonProperty("dkk")
    public String dkk;
    @JsonProperty("dot")
    public String dot;
    @JsonProperty("eos")
    public String eos;
    @JsonProperty("eth")
    public String eth;
    @JsonProperty("eur")
    public String eur;
    @JsonProperty("gbp")
    public String gbp;
    @JsonProperty("hkd")
    public String hkd;
    @JsonProperty("huf")
    public String huf;
    @JsonProperty("idr")
    public String idr;
    @JsonProperty("ils")
    public String ils;
    @JsonProperty("inr")
    public String inr;
    @JsonProperty("jpy")
    public String jpy;
    @JsonProperty("krw")
    public String krw;
    @JsonProperty("kwd")
    public String kwd;
    @JsonProperty("lkr")
    public String lkr;
    @JsonProperty("ltc")
    public String ltc;
    @JsonProperty("mmk")
    public String mmk;
    @JsonProperty("mxn")
    public String mxn;
    @JsonProperty("myr")
    public String myr;
    @JsonProperty("ngn")
    public String ngn;
    @JsonProperty("nok")
    public String nok;
    @JsonProperty("nzd")
    public String nzd;
    @JsonProperty("php")
    public String php;
    @JsonProperty("pkr")
    public String pkr;
    @JsonProperty("pln")
    public String pln;
    @JsonProperty("rub")
    public String rub;
    @JsonProperty("sar")
    public String sar;
    @JsonProperty("sek")
    public String sek;
    @JsonProperty("sgd")
    public String sgd;
    @JsonProperty("thb")
    public String thb;
    @JsonProperty("try")
    public String _try;
    @JsonProperty("twd")
    public String twd;
    @JsonProperty("uah")
    public String uah;
    @JsonProperty("usd")
    public String usd;
    @JsonProperty("vef")
    public String vef;
    @JsonProperty("vnd")
    public String vnd;
    @JsonProperty("xag")
    public String xag;
    @JsonProperty("xau")
    public String xau;
    @JsonProperty("xdr")
    public String xdr;
    @JsonProperty("xlm")
    public String xlm;
    @JsonProperty("xrp")
    public String xrp;
    @JsonProperty("yfi")
    public String yfi;
    @JsonProperty("zar")
    public String zar;
    @JsonProperty("bits")
    public String bits;
    @JsonProperty("link")
    public String link;
    @JsonProperty("sats")
    public String sats;

}

@JsonInclude(JsonInclude.Include.NON_NULL)
class Atl {

    @JsonProperty("aed")
    public float aed;
    @JsonProperty("ars")
    public float ars;
    @JsonProperty("aud")
    public float aud;
    @JsonProperty("bch")
    public float bch;
    @JsonProperty("bdt")
    public float bdt;
    @JsonProperty("bhd")
    public float bhd;
    @JsonProperty("bmd")
    public float bmd;
    @JsonProperty("bnb")
    public float bnb;
    @JsonProperty("brl")
    public float brl;
    @JsonProperty("btc")
    public float btc;
    @JsonProperty("cad")
    public float cad;
    @JsonProperty("chf")
    public float chf;
    @JsonProperty("clp")
    public float clp;
    @JsonProperty("cny")
    public float cny;
    @JsonProperty("czk")
    public float czk;
    @JsonProperty("dkk")
    public float dkk;
    @JsonProperty("dot")
    public float dot;
    @JsonProperty("eos")
    public float eos;
    @JsonProperty("eth")
    public float eth;
    @JsonProperty("eur")
    public float eur;
    @JsonProperty("gbp")
    public float gbp;
    @JsonProperty("hkd")
    public float hkd;
    @JsonProperty("huf")
    public float huf;
    @JsonProperty("idr")
    public float idr;
    @JsonProperty("ils")
    public float ils;
    @JsonProperty("inr")
    public float inr;
    @JsonProperty("jpy")
    public float jpy;
    @JsonProperty("krw")
    public float krw;
    @JsonProperty("kwd")
    public float kwd;
    @JsonProperty("lkr")
    public float lkr;
    @JsonProperty("ltc")
    public float ltc;
    @JsonProperty("mmk")
    public float mmk;
    @JsonProperty("mxn")
    public float mxn;
    @JsonProperty("myr")
    public float myr;
    @JsonProperty("ngn")
    public float ngn;
    @JsonProperty("nok")
    public float nok;
    @JsonProperty("nzd")
    public float nzd;
    @JsonProperty("php")
    public float php;
    @JsonProperty("pkr")
    public float pkr;
    @JsonProperty("pln")
    public float pln;
    @JsonProperty("rub")
    public float rub;
    @JsonProperty("sar")
    public float sar;
    @JsonProperty("sek")
    public float sek;
    @JsonProperty("sgd")
    public float sgd;
    @JsonProperty("thb")
    public float thb;
    @JsonProperty("try")
    public float _try;
    @JsonProperty("twd")
    public float twd;
    @JsonProperty("uah")
    public float uah;
    @JsonProperty("usd")
    public float usd;
    @JsonProperty("vef")
    public float vef;
    @JsonProperty("vnd")
    public float vnd;
    @JsonProperty("xag")
    public float xag;
    @JsonProperty("xau")
    public float xau;
    @JsonProperty("xdr")
    public float xdr;
    @JsonProperty("xlm")
    public float xlm;
    @JsonProperty("xrp")
    public float xrp;
    @JsonProperty("yfi")
    public float yfi;
    @JsonProperty("zar")
    public float zar;
    @JsonProperty("bits")
    public float bits;
    @JsonProperty("link")
    public float link;
    @JsonProperty("sats")
    public float sats;

}

@JsonInclude(JsonInclude.Include.NON_NULL)
class AtlChangePercentage {

    @JsonProperty("aed")
    public float aed;
    @JsonProperty("ars")
    public float ars;
    @JsonProperty("aud")
    public float aud;
    @JsonProperty("bch")
    public float bch;
    @JsonProperty("bdt")
    public float bdt;
    @JsonProperty("bhd")
    public float bhd;
    @JsonProperty("bmd")
    public float bmd;
    @JsonProperty("bnb")
    public float bnb;
    @JsonProperty("brl")
    public float brl;
    @JsonProperty("btc")
    public float btc;
    @JsonProperty("cad")
    public float cad;
    @JsonProperty("chf")
    public float chf;
    @JsonProperty("clp")
    public float clp;
    @JsonProperty("cny")
    public float cny;
    @JsonProperty("czk")
    public float czk;
    @JsonProperty("dkk")
    public float dkk;
    @JsonProperty("dot")
    public float dot;
    @JsonProperty("eos")
    public float eos;
    @JsonProperty("eth")
    public float eth;
    @JsonProperty("eur")
    public float eur;
    @JsonProperty("gbp")
    public float gbp;
    @JsonProperty("hkd")
    public float hkd;
    @JsonProperty("huf")
    public float huf;
    @JsonProperty("idr")
    public float idr;
    @JsonProperty("ils")
    public float ils;
    @JsonProperty("inr")
    public float inr;
    @JsonProperty("jpy")
    public float jpy;
    @JsonProperty("krw")
    public float krw;
    @JsonProperty("kwd")
    public float kwd;
    @JsonProperty("lkr")
    public float lkr;
    @JsonProperty("ltc")
    public float ltc;
    @JsonProperty("mmk")
    public float mmk;
    @JsonProperty("mxn")
    public float mxn;
    @JsonProperty("myr")
    public float myr;
    @JsonProperty("ngn")
    public float ngn;
    @JsonProperty("nok")
    public float nok;
    @JsonProperty("nzd")
    public float nzd;
    @JsonProperty("php")
    public float php;
    @JsonProperty("pkr")
    public float pkr;
    @JsonProperty("pln")
    public float pln;
    @JsonProperty("rub")
    public float rub;
    @JsonProperty("sar")
    public float sar;
    @JsonProperty("sek")
    public float sek;
    @JsonProperty("sgd")
    public float sgd;
    @JsonProperty("thb")
    public float thb;
    @JsonProperty("try")
    public float _try;
    @JsonProperty("twd")
    public float twd;
    @JsonProperty("uah")
    public float uah;
    @JsonProperty("usd")
    public float usd;
    @JsonProperty("vef")
    public float vef;
    @JsonProperty("vnd")
    public float vnd;
    @JsonProperty("xag")
    public float xag;
    @JsonProperty("xau")
    public float xau;
    @JsonProperty("xdr")
    public float xdr;
    @JsonProperty("xlm")
    public float xlm;
    @JsonProperty("xrp")
    public float xrp;
    @JsonProperty("yfi")
    public float yfi;
    @JsonProperty("zar")
    public float zar;
    @JsonProperty("bits")
    public float bits;
    @JsonProperty("link")
    public float link;
    @JsonProperty("sats")
    public float sats;

}

@JsonInclude(JsonInclude.Include.NON_NULL)
class AtlDate {

    @JsonProperty("aed")
    public String aed;
    @JsonProperty("ars")
    public String ars;
    @JsonProperty("aud")
    public String aud;
    @JsonProperty("bch")
    public String bch;
    @JsonProperty("bdt")
    public String bdt;
    @JsonProperty("bhd")
    public String bhd;
    @JsonProperty("bmd")
    public String bmd;
    @JsonProperty("bnb")
    public String bnb;
    @JsonProperty("brl")
    public String brl;
    @JsonProperty("btc")
    public String btc;
    @JsonProperty("cad")
    public String cad;
    @JsonProperty("chf")
    public String chf;
    @JsonProperty("clp")
    public String clp;
    @JsonProperty("cny")
    public String cny;
    @JsonProperty("czk")
    public String czk;
    @JsonProperty("dkk")
    public String dkk;
    @JsonProperty("dot")
    public String dot;
    @JsonProperty("eos")
    public String eos;
    @JsonProperty("eth")
    public String eth;
    @JsonProperty("eur")
    public String eur;
    @JsonProperty("gbp")
    public String gbp;
    @JsonProperty("hkd")
    public String hkd;
    @JsonProperty("huf")
    public String huf;
    @JsonProperty("idr")
    public String idr;
    @JsonProperty("ils")
    public String ils;
    @JsonProperty("inr")
    public String inr;
    @JsonProperty("jpy")
    public String jpy;
    @JsonProperty("krw")
    public String krw;
    @JsonProperty("kwd")
    public String kwd;
    @JsonProperty("lkr")
    public String lkr;
    @JsonProperty("ltc")
    public String ltc;
    @JsonProperty("mmk")
    public String mmk;
    @JsonProperty("mxn")
    public String mxn;
    @JsonProperty("myr")
    public String myr;
    @JsonProperty("ngn")
    public String ngn;
    @JsonProperty("nok")
    public String nok;
    @JsonProperty("nzd")
    public String nzd;
    @JsonProperty("php")
    public String php;
    @JsonProperty("pkr")
    public String pkr;
    @JsonProperty("pln")
    public String pln;
    @JsonProperty("rub")
    public String rub;
    @JsonProperty("sar")
    public String sar;
    @JsonProperty("sek")
    public String sek;
    @JsonProperty("sgd")
    public String sgd;
    @JsonProperty("thb")
    public String thb;
    @JsonProperty("try")
    public String _try;
    @JsonProperty("twd")
    public String twd;
    @JsonProperty("uah")
    public String uah;
    @JsonProperty("usd")
    public String usd;
    @JsonProperty("vef")
    public String vef;
    @JsonProperty("vnd")
    public String vnd;
    @JsonProperty("xag")
    public String xag;
    @JsonProperty("xau")
    public String xau;
    @JsonProperty("xdr")
    public String xdr;
    @JsonProperty("xlm")
    public String xlm;
    @JsonProperty("xrp")
    public String xrp;
    @JsonProperty("yfi")
    public String yfi;
    @JsonProperty("zar")
    public String zar;
    @JsonProperty("bits")
    public String bits;
    @JsonProperty("link")
    public String link;
    @JsonProperty("sats")
    public String sats;

}

@JsonInclude(JsonInclude.Include.NON_NULL)
class CodeAdditionsDeletions4Weeks {

    @JsonProperty("additions")
    public Object additions;
    @JsonProperty("deletions")
    public Object deletions;

}

@JsonInclude(JsonInclude.Include.NON_NULL)
class CommunityData {

    @JsonProperty("facebook_likes")
    public Object facebookLikes;
    @JsonProperty("twitter_followers")
    public long twitterFollowers;
    @JsonProperty("reddit_average_posts_48h")
    public float redditAveragePosts48h;
    @JsonProperty("reddit_average_comments_48h")
    public float redditAverageComments48h;
    @JsonProperty("reddit_subscribers")
    public long redditSubscribers;
    @JsonProperty("reddit_accounts_active_48h")
    public long redditAccountsActive48h;
    @JsonProperty("telegram_channel_user_count")
    public Object telegramChannelUserCount;

}


@JsonInclude(JsonInclude.Include.NON_NULL)
class ConvertedLast {

    @JsonProperty("btc")
    public float btc;
    @JsonProperty("eth")
    public float eth;
    @JsonProperty("usd")
    public float usd;

}

@JsonInclude(JsonInclude.Include.NON_NULL)
class Description {

    @JsonProperty("en")
    public String en;
    @JsonProperty("de")
    public String de;
    @JsonProperty("es")
    public String es;
    @JsonProperty("fr")
    public String fr;
    @JsonProperty("it")
    public String it;
    @JsonProperty("pl")
    public String pl;
    @JsonProperty("ro")
    public String ro;
    @JsonProperty("hu")
    public String hu;
    @JsonProperty("nl")
    public String nl;
    @JsonProperty("pt")
    public String pt;
    @JsonProperty("sv")
    public String sv;
    @JsonProperty("vi")
    public String vi;
    @JsonProperty("tr")
    public String tr;
    @JsonProperty("ru")
    public String ru;
    @JsonProperty("ja")
    public String ja;
    @JsonProperty("zh")
    public String zh;
    @JsonProperty("zh-tw")
    public String zhTw;
    @JsonProperty("ko")
    public String ko;
    @JsonProperty("ar")
    public String ar;
    @JsonProperty("th")
    public String th;
    @JsonProperty("id")
    public String id;

}

@JsonInclude(JsonInclude.Include.NON_NULL)
class DeveloperData {

    @JsonProperty("forks")
    public long forks;
    @JsonProperty("stars")
    public long stars;
    @JsonProperty("subscribers")
    public long subscribers;
    @JsonProperty("total_issues")
    public long totalIssues;
    @JsonProperty("closed_issues")
    public long closedIssues;
    @JsonProperty("pull_requests_merged")
    public long pullRequestsMerged;
    @JsonProperty("pull_request_contributors")
    public long pullRequestContributors;
    @JsonProperty("code_additions_deletions_4_weeks")
    public CodeAdditionsDeletions4Weeks codeAdditionsDeletions4Weeks;
    @JsonProperty("commit_count_4_weeks")
    public long commitCount4Weeks;
    @JsonProperty("last_4_weeks_commit_activity_series")
    public List<Object> last4WeeksCommitActivitySeries = null;

}

@JsonInclude(JsonInclude.Include.NON_NULL)
public class CoingeckoCoinDetail {

    @JsonProperty("id")
    public String id;
    @JsonProperty("symbol")
    public String symbol;
    @JsonProperty("name")
    public String name;
    @JsonProperty("asset_platform_id")
    public String assetPlatformId;
    @JsonProperty("platforms")
    public Platforms platforms;
    @JsonProperty("block_time_in_minutes")
    public long blockTimeInMinutes;
    @JsonProperty("hashing_algorithm")
    public Object hashingAlgorithm;
    @JsonProperty("categories")
    public List<String> categories = null;
    @JsonProperty("public_notice")
    public Object publicNotice;
    @JsonProperty("additional_notices")
    public List<String> additionalNotices = null;
    @JsonProperty("localization")
    public Localization localization;
    @JsonProperty("description")
    public Description description;
    @JsonProperty("links")
    public Links links;
    @JsonProperty("image")
    public Image image;
    @JsonProperty("country_origin")
    public String countryOrigin;
    @JsonProperty("genesis_date")
    public Object genesisDate;
    @JsonProperty("contract_address")
    public String contractAddress;
    @JsonProperty("sentiment_votes_up_percentage")
    public Object sentimentVotesUpPercentage;
    @JsonProperty("sentiment_votes_down_percentage")
    public Object sentimentVotesDownPercentage;
    @JsonProperty("market_cap_rank")
    public Object marketCapRank;
    @JsonProperty("coingecko_rank")
    public long coingeckoRank;
    @JsonProperty("coingecko_score")
    public float coingeckoScore;
    @JsonProperty("developer_score")
    public float developerScore;
    @JsonProperty("community_score")
    public float communityScore;
    @JsonProperty("liquidity_score")
    public float liquidityScore;
    @JsonProperty("public_interest_score")
    public float publicInterestScore;
    @JsonProperty("market_data")
    public MarketData marketData;
    @JsonProperty("community_data")
    public CommunityData communityData;
    @JsonProperty("developer_data")
    public DeveloperData developerData;
    @JsonProperty("public_interest_stats")
    public PublicInterestStats publicInterestStats;
    @JsonProperty("status_updates")
    public List<Object> statusUpdates = null;
    @JsonProperty("last_updated")
    public String lastUpdated;
    @JsonProperty("tickers")
    public List<Ticker> tickers = null;

}

@JsonInclude(JsonInclude.Include.NON_NULL)
class FullyDilutedValuation {


}

@JsonInclude(JsonInclude.Include.NON_NULL)
class High24h {

    @JsonProperty("aed")
    public float aed;
    @JsonProperty("ars")
    public float ars;
    @JsonProperty("aud")
    public float aud;
    @JsonProperty("bch")
    public float bch;
    @JsonProperty("bdt")
    public float bdt;
    @JsonProperty("bhd")
    public float bhd;
    @JsonProperty("bmd")
    public float bmd;
    @JsonProperty("bnb")
    public float bnb;
    @JsonProperty("brl")
    public float brl;
    @JsonProperty("btc")
    public float btc;
    @JsonProperty("cad")
    public float cad;
    @JsonProperty("chf")
    public float chf;
    @JsonProperty("clp")
    public float clp;
    @JsonProperty("cny")
    public float cny;
    @JsonProperty("czk")
    public float czk;
    @JsonProperty("dkk")
    public float dkk;
    @JsonProperty("dot")
    public float dot;
    @JsonProperty("eos")
    public float eos;
    @JsonProperty("eth")
    public float eth;
    @JsonProperty("eur")
    public float eur;
    @JsonProperty("gbp")
    public float gbp;
    @JsonProperty("hkd")
    public float hkd;
    @JsonProperty("huf")
    public float huf;
    @JsonProperty("idr")
    public float idr;
    @JsonProperty("ils")
    public float ils;
    @JsonProperty("inr")
    public float inr;
    @JsonProperty("jpy")
    public float jpy;
    @JsonProperty("krw")
    public float krw;
    @JsonProperty("kwd")
    public float kwd;
    @JsonProperty("lkr")
    public float lkr;
    @JsonProperty("ltc")
    public float ltc;
    @JsonProperty("mmk")
    public float mmk;
    @JsonProperty("mxn")
    public float mxn;
    @JsonProperty("myr")
    public float myr;
    @JsonProperty("ngn")
    public float ngn;
    @JsonProperty("nok")
    public float nok;
    @JsonProperty("nzd")
    public float nzd;
    @JsonProperty("php")
    public float php;
    @JsonProperty("pkr")
    public float pkr;
    @JsonProperty("pln")
    public float pln;
    @JsonProperty("rub")
    public float rub;
    @JsonProperty("sar")
    public float sar;
    @JsonProperty("sek")
    public float sek;
    @JsonProperty("sgd")
    public float sgd;
    @JsonProperty("thb")
    public float thb;
    @JsonProperty("try")
    public float _try;
    @JsonProperty("twd")
    public float twd;
    @JsonProperty("uah")
    public float uah;
    @JsonProperty("usd")
    public float usd;
    @JsonProperty("vef")
    public float vef;
    @JsonProperty("vnd")
    public long vnd;
    @JsonProperty("xag")
    public float xag;
    @JsonProperty("xau")
    public float xau;
    @JsonProperty("xdr")
    public float xdr;
    @JsonProperty("xlm")
    public float xlm;
    @JsonProperty("xrp")
    public float xrp;
    @JsonProperty("yfi")
    public float yfi;
    @JsonProperty("zar")
    public float zar;
    @JsonProperty("bits")
    public float bits;
    @JsonProperty("link")
    public float link;
    @JsonProperty("sats")
    public float sats;

}

@JsonInclude(JsonInclude.Include.NON_NULL)
class Image {

    @JsonProperty("thumb")
    public String thumb;
    @JsonProperty("small")
    public String small;
    @JsonProperty("large")
    public String large;

}

@JsonInclude(JsonInclude.Include.NON_NULL)
class Links {

    @JsonProperty("homepage")
    public List<String> homepage = null;
    @JsonProperty("blockchain_site")
    public List<String> blockchainSite = null;
    @JsonProperty("official_forum_url")
    public List<String> officialForumUrl = null;
    @JsonProperty("chat_url")
    public List<String> chatUrl = null;
    @JsonProperty("announcement_url")
    public List<String> announcementUrl = null;
    @JsonProperty("twitter_screen_name")
    public String twitterScreenName;
    @JsonProperty("facebook_username")
    public String facebookUsername;
    @JsonProperty("bitcointalk_thread_identifier")
    public Object bitcointalkThreadIdentifier;
    @JsonProperty("telegram_channel_identifier")
    public String telegramChannelIdentifier;
    @JsonProperty("subreddit_url")
    public Object subredditUrl;
    @JsonProperty("repos_url")
    public ReposUrl reposUrl;

}

@JsonInclude(JsonInclude.Include.NON_NULL)
class Localization {

    @JsonProperty("en")
    public String en;
    @JsonProperty("de")
    public String de;
    @JsonProperty("es")
    public String es;
    @JsonProperty("fr")
    public String fr;
    @JsonProperty("it")
    public String it;
    @JsonProperty("pl")
    public String pl;
    @JsonProperty("ro")
    public String ro;
    @JsonProperty("hu")
    public String hu;
    @JsonProperty("nl")
    public String nl;
    @JsonProperty("pt")
    public String pt;
    @JsonProperty("sv")
    public String sv;
    @JsonProperty("vi")
    public String vi;
    @JsonProperty("tr")
    public String tr;
    @JsonProperty("ru")
    public String ru;
    @JsonProperty("ja")
    public String ja;
    @JsonProperty("zh")
    public String zh;
    @JsonProperty("zh-tw")
    public String zhTw;
    @JsonProperty("ko")
    public String ko;
    @JsonProperty("ar")
    public String ar;
    @JsonProperty("th")
    public String th;
    @JsonProperty("id")
    public String id;

}

@JsonInclude(JsonInclude.Include.NON_NULL)
class Low24h {

    @JsonProperty("aed")
    public float aed;
    @JsonProperty("ars")
    public float ars;
    @JsonProperty("aud")
    public float aud;
    @JsonProperty("bch")
    public float bch;
    @JsonProperty("bdt")
    public float bdt;
    @JsonProperty("bhd")
    public float bhd;
    @JsonProperty("bmd")
    public float bmd;
    @JsonProperty("bnb")
    public float bnb;
    @JsonProperty("brl")
    public float brl;
    @JsonProperty("btc")
    public float btc;
    @JsonProperty("cad")
    public float cad;
    @JsonProperty("chf")
    public float chf;
    @JsonProperty("clp")
    public float clp;
    @JsonProperty("cny")
    public float cny;
    @JsonProperty("czk")
    public float czk;
    @JsonProperty("dkk")
    public float dkk;
    @JsonProperty("dot")
    public float dot;
    @JsonProperty("eos")
    public float eos;
    @JsonProperty("eth")
    public float eth;
    @JsonProperty("eur")
    public float eur;
    @JsonProperty("gbp")
    public float gbp;
    @JsonProperty("hkd")
    public float hkd;
    @JsonProperty("huf")
    public float huf;
    @JsonProperty("idr")
    public float idr;
    @JsonProperty("ils")
    public float ils;
    @JsonProperty("inr")
    public float inr;
    @JsonProperty("jpy")
    public float jpy;
    @JsonProperty("krw")
    public float krw;
    @JsonProperty("kwd")
    public float kwd;
    @JsonProperty("lkr")
    public float lkr;
    @JsonProperty("ltc")
    public float ltc;
    @JsonProperty("mmk")
    public float mmk;
    @JsonProperty("mxn")
    public float mxn;
    @JsonProperty("myr")
    public float myr;
    @JsonProperty("ngn")
    public float ngn;
    @JsonProperty("nok")
    public float nok;
    @JsonProperty("nzd")
    public float nzd;
    @JsonProperty("php")
    public float php;
    @JsonProperty("pkr")
    public float pkr;
    @JsonProperty("pln")
    public float pln;
    @JsonProperty("rub")
    public float rub;
    @JsonProperty("sar")
    public float sar;
    @JsonProperty("sek")
    public float sek;
    @JsonProperty("sgd")
    public float sgd;
    @JsonProperty("thb")
    public float thb;
    @JsonProperty("try")
    public float _try;
    @JsonProperty("twd")
    public float twd;
    @JsonProperty("uah")
    public float uah;
    @JsonProperty("usd")
    public float usd;
    @JsonProperty("vef")
    public float vef;
    @JsonProperty("vnd")
    public long vnd;
    @JsonProperty("xag")
    public float xag;
    @JsonProperty("xau")
    public float xau;
    @JsonProperty("xdr")
    public float xdr;
    @JsonProperty("xlm")
    public float xlm;
    @JsonProperty("xrp")
    public float xrp;
    @JsonProperty("yfi")
    public float yfi;
    @JsonProperty("zar")
    public float zar;
    @JsonProperty("bits")
    public float bits;
    @JsonProperty("link")
    public float link;
    @JsonProperty("sats")
    public float sats;

}

@JsonInclude(JsonInclude.Include.NON_NULL)
class MarketCap {

    @JsonProperty("aed")
    public float aed;
    @JsonProperty("ars")
    public float ars;
    @JsonProperty("aud")
    public float aud;
    @JsonProperty("bch")
    public float bch;
    @JsonProperty("bdt")
    public float bdt;
    @JsonProperty("bhd")
    public float bhd;
    @JsonProperty("bmd")
    public float bmd;
    @JsonProperty("bnb")
    public float bnb;
    @JsonProperty("brl")
    public float brl;
    @JsonProperty("btc")
    public float btc;
    @JsonProperty("cad")
    public float cad;
    @JsonProperty("chf")
    public float chf;
    @JsonProperty("clp")
    public float clp;
    @JsonProperty("cny")
    public float cny;
    @JsonProperty("czk")
    public float czk;
    @JsonProperty("dkk")
    public float dkk;
    @JsonProperty("dot")
    public float dot;
    @JsonProperty("eos")
    public float eos;
    @JsonProperty("eth")
    public float eth;
    @JsonProperty("eur")
    public float eur;
    @JsonProperty("gbp")
    public float gbp;
    @JsonProperty("hkd")
    public float hkd;
    @JsonProperty("huf")
    public float huf;
    @JsonProperty("idr")
    public float idr;
    @JsonProperty("ils")
    public float ils;
    @JsonProperty("inr")
    public float inr;
    @JsonProperty("jpy")
    public float jpy;
    @JsonProperty("krw")
    public float krw;
    @JsonProperty("kwd")
    public float kwd;
    @JsonProperty("lkr")
    public float lkr;
    @JsonProperty("ltc")
    public float ltc;
    @JsonProperty("mmk")
    public float mmk;
    @JsonProperty("mxn")
    public float mxn;
    @JsonProperty("myr")
    public float myr;
    @JsonProperty("ngn")
    public float ngn;
    @JsonProperty("nok")
    public float nok;
    @JsonProperty("nzd")
    public float nzd;
    @JsonProperty("php")
    public float php;
    @JsonProperty("pkr")
    public float pkr;
    @JsonProperty("pln")
    public float pln;
    @JsonProperty("rub")
    public float rub;
    @JsonProperty("sar")
    public float sar;
    @JsonProperty("sek")
    public float sek;
    @JsonProperty("sgd")
    public float sgd;
    @JsonProperty("thb")
    public float thb;
    @JsonProperty("try")
    public float _try;
    @JsonProperty("twd")
    public float twd;
    @JsonProperty("uah")
    public float uah;
    @JsonProperty("usd")
    public float usd;
    @JsonProperty("vef")
    public float vef;
    @JsonProperty("vnd")
    public float vnd;
    @JsonProperty("xag")
    public float xag;
    @JsonProperty("xau")
    public float xau;
    @JsonProperty("xdr")
    public float xdr;
    @JsonProperty("xlm")
    public float xlm;
    @JsonProperty("xrp")
    public float xrp;
    @JsonProperty("yfi")
    public float yfi;
    @JsonProperty("zar")
    public float zar;
    @JsonProperty("bits")
    public float bits;
    @JsonProperty("link")
    public float link;
    @JsonProperty("sats")
    public float sats;

}

@JsonInclude(JsonInclude.Include.NON_NULL)
class MarketCapChange24hInCurrency {

    @JsonProperty("aed")
    public float aed;
    @JsonProperty("ars")
    public float ars;
    @JsonProperty("aud")
    public float aud;
    @JsonProperty("bch")
    public float bch;
    @JsonProperty("bdt")
    public float bdt;
    @JsonProperty("bhd")
    public float bhd;
    @JsonProperty("bmd")
    public float bmd;
    @JsonProperty("bnb")
    public float bnb;
    @JsonProperty("brl")
    public float brl;
    @JsonProperty("btc")
    public float btc;
    @JsonProperty("cad")
    public float cad;
    @JsonProperty("chf")
    public float chf;
    @JsonProperty("clp")
    public float clp;
    @JsonProperty("cny")
    public float cny;
    @JsonProperty("czk")
    public float czk;
    @JsonProperty("dkk")
    public float dkk;
    @JsonProperty("dot")
    public float dot;
    @JsonProperty("eos")
    public float eos;
    @JsonProperty("eth")
    public float eth;
    @JsonProperty("eur")
    public float eur;
    @JsonProperty("gbp")
    public float gbp;
    @JsonProperty("hkd")
    public float hkd;
    @JsonProperty("huf")
    public float huf;
    @JsonProperty("idr")
    public float idr;
    @JsonProperty("ils")
    public float ils;
    @JsonProperty("inr")
    public float inr;
    @JsonProperty("jpy")
    public float jpy;
    @JsonProperty("krw")
    public float krw;
    @JsonProperty("kwd")
    public float kwd;
    @JsonProperty("lkr")
    public float lkr;
    @JsonProperty("ltc")
    public float ltc;
    @JsonProperty("mmk")
    public float mmk;
    @JsonProperty("mxn")
    public float mxn;
    @JsonProperty("myr")
    public float myr;
    @JsonProperty("ngn")
    public float ngn;
    @JsonProperty("nok")
    public float nok;
    @JsonProperty("nzd")
    public float nzd;
    @JsonProperty("php")
    public float php;
    @JsonProperty("pkr")
    public float pkr;
    @JsonProperty("pln")
    public float pln;
    @JsonProperty("rub")
    public float rub;
    @JsonProperty("sar")
    public float sar;
    @JsonProperty("sek")
    public float sek;
    @JsonProperty("sgd")
    public float sgd;
    @JsonProperty("thb")
    public float thb;
    @JsonProperty("try")
    public float _try;
    @JsonProperty("twd")
    public float twd;
    @JsonProperty("uah")
    public float uah;
    @JsonProperty("usd")
    public float usd;
    @JsonProperty("vef")
    public float vef;
    @JsonProperty("vnd")
    public float vnd;
    @JsonProperty("xag")
    public float xag;
    @JsonProperty("xau")
    public float xau;
    @JsonProperty("xdr")
    public float xdr;
    @JsonProperty("xlm")
    public float xlm;
    @JsonProperty("xrp")
    public float xrp;
    @JsonProperty("yfi")
    public float yfi;
    @JsonProperty("zar")
    public float zar;
    @JsonProperty("bits")
    public float bits;
    @JsonProperty("link")
    public float link;
    @JsonProperty("sats")
    public float sats;

}

@JsonInclude(JsonInclude.Include.NON_NULL)
class MarketCapChangePercentage24hInCurrency {

    @JsonProperty("aed")
    public float aed;
    @JsonProperty("ars")
    public float ars;
    @JsonProperty("aud")
    public float aud;
    @JsonProperty("bch")
    public float bch;
    @JsonProperty("bdt")
    public float bdt;
    @JsonProperty("bhd")
    public float bhd;
    @JsonProperty("bmd")
    public float bmd;
    @JsonProperty("bnb")
    public float bnb;
    @JsonProperty("brl")
    public float brl;
    @JsonProperty("btc")
    public float btc;
    @JsonProperty("cad")
    public float cad;
    @JsonProperty("chf")
    public float chf;
    @JsonProperty("clp")
    public float clp;
    @JsonProperty("cny")
    public float cny;
    @JsonProperty("czk")
    public float czk;
    @JsonProperty("dkk")
    public float dkk;
    @JsonProperty("dot")
    public float dot;
    @JsonProperty("eos")
    public float eos;
    @JsonProperty("eth")
    public float eth;
    @JsonProperty("eur")
    public float eur;
    @JsonProperty("gbp")
    public float gbp;
    @JsonProperty("hkd")
    public float hkd;
    @JsonProperty("huf")
    public float huf;
    @JsonProperty("idr")
    public float idr;
    @JsonProperty("ils")
    public float ils;
    @JsonProperty("inr")
    public float inr;
    @JsonProperty("jpy")
    public float jpy;
    @JsonProperty("krw")
    public float krw;
    @JsonProperty("kwd")
    public float kwd;
    @JsonProperty("lkr")
    public float lkr;
    @JsonProperty("ltc")
    public float ltc;
    @JsonProperty("mmk")
    public float mmk;
    @JsonProperty("mxn")
    public float mxn;
    @JsonProperty("myr")
    public float myr;
    @JsonProperty("ngn")
    public float ngn;
    @JsonProperty("nok")
    public float nok;
    @JsonProperty("nzd")
    public float nzd;
    @JsonProperty("php")
    public float php;
    @JsonProperty("pkr")
    public float pkr;
    @JsonProperty("pln")
    public float pln;
    @JsonProperty("rub")
    public float rub;
    @JsonProperty("sar")
    public float sar;
    @JsonProperty("sek")
    public float sek;
    @JsonProperty("sgd")
    public float sgd;
    @JsonProperty("thb")
    public float thb;
    @JsonProperty("try")
    public float _try;
    @JsonProperty("twd")
    public float twd;
    @JsonProperty("uah")
    public float uah;
    @JsonProperty("usd")
    public float usd;
    @JsonProperty("vef")
    public float vef;
    @JsonProperty("vnd")
    public float vnd;
    @JsonProperty("xag")
    public float xag;
    @JsonProperty("xau")
    public float xau;
    @JsonProperty("xdr")
    public float xdr;
    @JsonProperty("xlm")
    public float xlm;
    @JsonProperty("xrp")
    public float xrp;
    @JsonProperty("yfi")
    public float yfi;
    @JsonProperty("zar")
    public float zar;
    @JsonProperty("bits")
    public float bits;
    @JsonProperty("link")
    public float link;
    @JsonProperty("sats")
    public float sats;

}

@JsonInclude(JsonInclude.Include.NON_NULL)
class Platforms {

    @JsonProperty("polygon-pos")
    public String polygonPos;

}

@JsonInclude(JsonInclude.Include.NON_NULL)
class PriceChange24hInCurrency {

    @JsonProperty("aed")
    public float aed;
    @JsonProperty("ars")
    public float ars;
    @JsonProperty("aud")
    public float aud;
    @JsonProperty("bch")
    public float bch;
    @JsonProperty("bdt")
    public float bdt;
    @JsonProperty("bhd")
    public float bhd;
    @JsonProperty("bmd")
    public float bmd;
    @JsonProperty("bnb")
    public float bnb;
    @JsonProperty("brl")
    public float brl;
    @JsonProperty("btc")
    public float btc;
    @JsonProperty("cad")
    public float cad;
    @JsonProperty("chf")
    public float chf;
    @JsonProperty("clp")
    public float clp;
    @JsonProperty("cny")
    public float cny;
    @JsonProperty("czk")
    public float czk;
    @JsonProperty("dkk")
    public float dkk;
    @JsonProperty("dot")
    public float dot;
    @JsonProperty("eos")
    public float eos;
    @JsonProperty("eth")
    public float eth;
    @JsonProperty("eur")
    public float eur;
    @JsonProperty("gbp")
    public float gbp;
    @JsonProperty("hkd")
    public float hkd;
    @JsonProperty("huf")
    public float huf;
    @JsonProperty("idr")
    public float idr;
    @JsonProperty("ils")
    public float ils;
    @JsonProperty("inr")
    public float inr;
    @JsonProperty("jpy")
    public float jpy;
    @JsonProperty("krw")
    public float krw;
    @JsonProperty("kwd")
    public float kwd;
    @JsonProperty("lkr")
    public float lkr;
    @JsonProperty("ltc")
    public float ltc;
    @JsonProperty("mmk")
    public float mmk;
    @JsonProperty("mxn")
    public float mxn;
    @JsonProperty("myr")
    public float myr;
    @JsonProperty("ngn")
    public float ngn;
    @JsonProperty("nok")
    public float nok;
    @JsonProperty("nzd")
    public float nzd;
    @JsonProperty("php")
    public float php;
    @JsonProperty("pkr")
    public float pkr;
    @JsonProperty("pln")
    public float pln;
    @JsonProperty("rub")
    public float rub;
    @JsonProperty("sar")
    public float sar;
    @JsonProperty("sek")
    public float sek;
    @JsonProperty("sgd")
    public float sgd;
    @JsonProperty("thb")
    public float thb;
    @JsonProperty("try")
    public float _try;
    @JsonProperty("twd")
    public float twd;
    @JsonProperty("uah")
    public float uah;
    @JsonProperty("usd")
    public float usd;
    @JsonProperty("vef")
    public float vef;
    @JsonProperty("vnd")
    public float vnd;
    @JsonProperty("xag")
    public float xag;
    @JsonProperty("xau")
    public float xau;
    @JsonProperty("xdr")
    public float xdr;
    @JsonProperty("xlm")
    public float xlm;
    @JsonProperty("xrp")
    public float xrp;
    @JsonProperty("yfi")
    public float yfi;
    @JsonProperty("zar")
    public float zar;
    @JsonProperty("bits")
    public float bits;
    @JsonProperty("link")
    public float link;
    @JsonProperty("sats")
    public float sats;

}

@JsonInclude(JsonInclude.Include.NON_NULL)
class PriceChangePercentage14dInCurrency {

    @JsonProperty("aed")
    public float aed;
    @JsonProperty("ars")
    public float ars;
    @JsonProperty("aud")
    public float aud;
    @JsonProperty("bch")
    public float bch;
    @JsonProperty("bdt")
    public float bdt;
    @JsonProperty("bhd")
    public float bhd;
    @JsonProperty("bmd")
    public float bmd;
    @JsonProperty("bnb")
    public float bnb;
    @JsonProperty("brl")
    public float brl;
    @JsonProperty("btc")
    public float btc;
    @JsonProperty("cad")
    public float cad;
    @JsonProperty("chf")
    public float chf;
    @JsonProperty("clp")
    public float clp;
    @JsonProperty("cny")
    public float cny;
    @JsonProperty("czk")
    public float czk;
    @JsonProperty("dkk")
    public float dkk;
    @JsonProperty("dot")
    public float dot;
    @JsonProperty("eos")
    public float eos;
    @JsonProperty("eth")
    public float eth;
    @JsonProperty("eur")
    public float eur;
    @JsonProperty("gbp")
    public float gbp;
    @JsonProperty("hkd")
    public float hkd;
    @JsonProperty("huf")
    public float huf;
    @JsonProperty("idr")
    public float idr;
    @JsonProperty("ils")
    public float ils;
    @JsonProperty("inr")
    public float inr;
    @JsonProperty("jpy")
    public float jpy;
    @JsonProperty("krw")
    public float krw;
    @JsonProperty("kwd")
    public float kwd;
    @JsonProperty("lkr")
    public float lkr;
    @JsonProperty("ltc")
    public float ltc;
    @JsonProperty("mmk")
    public float mmk;
    @JsonProperty("mxn")
    public float mxn;
    @JsonProperty("myr")
    public float myr;
    @JsonProperty("ngn")
    public float ngn;
    @JsonProperty("nok")
    public float nok;
    @JsonProperty("nzd")
    public float nzd;
    @JsonProperty("php")
    public float php;
    @JsonProperty("pkr")
    public float pkr;
    @JsonProperty("pln")
    public float pln;
    @JsonProperty("rub")
    public float rub;
    @JsonProperty("sar")
    public float sar;
    @JsonProperty("sek")
    public float sek;
    @JsonProperty("sgd")
    public float sgd;
    @JsonProperty("thb")
    public float thb;
    @JsonProperty("try")
    public float _try;
    @JsonProperty("twd")
    public float twd;
    @JsonProperty("uah")
    public float uah;
    @JsonProperty("usd")
    public float usd;
    @JsonProperty("vef")
    public float vef;
    @JsonProperty("vnd")
    public float vnd;
    @JsonProperty("xag")
    public float xag;
    @JsonProperty("xau")
    public float xau;
    @JsonProperty("xdr")
    public float xdr;
    @JsonProperty("xlm")
    public float xlm;
    @JsonProperty("xrp")
    public float xrp;
    @JsonProperty("yfi")
    public float yfi;
    @JsonProperty("zar")
    public float zar;
    @JsonProperty("bits")
    public float bits;
    @JsonProperty("link")
    public float link;
    @JsonProperty("sats")
    public float sats;

}

@JsonInclude(JsonInclude.Include.NON_NULL)
class PriceChangePercentage1hInCurrency {

    @JsonProperty("aed")
    public float aed;
    @JsonProperty("ars")
    public float ars;
    @JsonProperty("aud")
    public float aud;
    @JsonProperty("bch")
    public float bch;
    @JsonProperty("bdt")
    public float bdt;
    @JsonProperty("bhd")
    public float bhd;
    @JsonProperty("bmd")
    public float bmd;
    @JsonProperty("bnb")
    public float bnb;
    @JsonProperty("brl")
    public float brl;
    @JsonProperty("btc")
    public float btc;
    @JsonProperty("cad")
    public float cad;
    @JsonProperty("chf")
    public float chf;
    @JsonProperty("clp")
    public float clp;
    @JsonProperty("cny")
    public float cny;
    @JsonProperty("czk")
    public float czk;
    @JsonProperty("dkk")
    public float dkk;
    @JsonProperty("dot")
    public float dot;
    @JsonProperty("eos")
    public float eos;
    @JsonProperty("eth")
    public float eth;
    @JsonProperty("eur")
    public float eur;
    @JsonProperty("gbp")
    public float gbp;
    @JsonProperty("hkd")
    public float hkd;
    @JsonProperty("huf")
    public float huf;
    @JsonProperty("idr")
    public float idr;
    @JsonProperty("ils")
    public float ils;
    @JsonProperty("inr")
    public float inr;
    @JsonProperty("jpy")
    public float jpy;
    @JsonProperty("krw")
    public float krw;
    @JsonProperty("kwd")
    public float kwd;
    @JsonProperty("lkr")
    public float lkr;
    @JsonProperty("ltc")
    public float ltc;
    @JsonProperty("mmk")
    public float mmk;
    @JsonProperty("mxn")
    public float mxn;
    @JsonProperty("myr")
    public float myr;
    @JsonProperty("ngn")
    public float ngn;
    @JsonProperty("nok")
    public float nok;
    @JsonProperty("nzd")
    public float nzd;
    @JsonProperty("php")
    public float php;
    @JsonProperty("pkr")
    public float pkr;
    @JsonProperty("pln")
    public float pln;
    @JsonProperty("rub")
    public float rub;
    @JsonProperty("sar")
    public float sar;
    @JsonProperty("sek")
    public float sek;
    @JsonProperty("sgd")
    public float sgd;
    @JsonProperty("thb")
    public float thb;
    @JsonProperty("try")
    public float _try;
    @JsonProperty("twd")
    public float twd;
    @JsonProperty("uah")
    public float uah;
    @JsonProperty("usd")
    public float usd;
    @JsonProperty("vef")
    public float vef;
    @JsonProperty("vnd")
    public float vnd;
    @JsonProperty("xag")
    public float xag;
    @JsonProperty("xau")
    public float xau;
    @JsonProperty("xdr")
    public float xdr;
    @JsonProperty("xlm")
    public float xlm;
    @JsonProperty("xrp")
    public float xrp;
    @JsonProperty("yfi")
    public float yfi;
    @JsonProperty("zar")
    public float zar;
    @JsonProperty("bits")
    public float bits;
    @JsonProperty("link")
    public float link;
    @JsonProperty("sats")
    public float sats;

}

@JsonInclude(JsonInclude.Include.NON_NULL)
class PriceChangePercentage1yInCurrency {


}

@JsonInclude(JsonInclude.Include.NON_NULL)
class PriceChangePercentage200dInCurrency {


}

@JsonInclude(JsonInclude.Include.NON_NULL)
class PriceChangePercentage24hInCurrency {

    @JsonProperty("aed")
    public float aed;
    @JsonProperty("ars")
    public float ars;
    @JsonProperty("aud")
    public float aud;
    @JsonProperty("bch")
    public float bch;
    @JsonProperty("bdt")
    public float bdt;
    @JsonProperty("bhd")
    public float bhd;
    @JsonProperty("bmd")
    public float bmd;
    @JsonProperty("bnb")
    public float bnb;
    @JsonProperty("brl")
    public float brl;
    @JsonProperty("btc")
    public float btc;
    @JsonProperty("cad")
    public float cad;
    @JsonProperty("chf")
    public float chf;
    @JsonProperty("clp")
    public float clp;
    @JsonProperty("cny")
    public float cny;
    @JsonProperty("czk")
    public float czk;
    @JsonProperty("dkk")
    public float dkk;
    @JsonProperty("dot")
    public float dot;
    @JsonProperty("eos")
    public float eos;
    @JsonProperty("eth")
    public float eth;
    @JsonProperty("eur")
    public float eur;
    @JsonProperty("gbp")
    public float gbp;
    @JsonProperty("hkd")
    public float hkd;
    @JsonProperty("huf")
    public float huf;
    @JsonProperty("idr")
    public float idr;
    @JsonProperty("ils")
    public float ils;
    @JsonProperty("inr")
    public float inr;
    @JsonProperty("jpy")
    public float jpy;
    @JsonProperty("krw")
    public float krw;
    @JsonProperty("kwd")
    public float kwd;
    @JsonProperty("lkr")
    public float lkr;
    @JsonProperty("ltc")
    public float ltc;
    @JsonProperty("mmk")
    public float mmk;
    @JsonProperty("mxn")
    public float mxn;
    @JsonProperty("myr")
    public float myr;
    @JsonProperty("ngn")
    public float ngn;
    @JsonProperty("nok")
    public float nok;
    @JsonProperty("nzd")
    public float nzd;
    @JsonProperty("php")
    public float php;
    @JsonProperty("pkr")
    public float pkr;
    @JsonProperty("pln")
    public float pln;
    @JsonProperty("rub")
    public float rub;
    @JsonProperty("sar")
    public float sar;
    @JsonProperty("sek")
    public float sek;
    @JsonProperty("sgd")
    public float sgd;
    @JsonProperty("thb")
    public float thb;
    @JsonProperty("try")
    public float _try;
    @JsonProperty("twd")
    public float twd;
    @JsonProperty("uah")
    public float uah;
    @JsonProperty("usd")
    public float usd;
    @JsonProperty("vef")
    public float vef;
    @JsonProperty("vnd")
    public float vnd;
    @JsonProperty("xag")
    public float xag;
    @JsonProperty("xau")
    public float xau;
    @JsonProperty("xdr")
    public float xdr;
    @JsonProperty("xlm")
    public float xlm;
    @JsonProperty("xrp")
    public float xrp;
    @JsonProperty("yfi")
    public float yfi;
    @JsonProperty("zar")
    public float zar;
    @JsonProperty("bits")
    public float bits;
    @JsonProperty("link")
    public float link;
    @JsonProperty("sats")
    public float sats;

}

@JsonInclude(JsonInclude.Include.NON_NULL)
class PriceChangePercentage30dInCurrency {

    @JsonProperty("aed")
    public float aed;
    @JsonProperty("ars")
    public float ars;
    @JsonProperty("aud")
    public float aud;
    @JsonProperty("bch")
    public float bch;
    @JsonProperty("bdt")
    public float bdt;
    @JsonProperty("bhd")
    public float bhd;
    @JsonProperty("bmd")
    public float bmd;
    @JsonProperty("bnb")
    public float bnb;
    @JsonProperty("brl")
    public float brl;
    @JsonProperty("btc")
    public float btc;
    @JsonProperty("cad")
    public float cad;
    @JsonProperty("chf")
    public float chf;
    @JsonProperty("clp")
    public float clp;
    @JsonProperty("cny")
    public float cny;
    @JsonProperty("czk")
    public float czk;
    @JsonProperty("dkk")
    public float dkk;
    @JsonProperty("dot")
    public float dot;
    @JsonProperty("eos")
    public float eos;
    @JsonProperty("eth")
    public float eth;
    @JsonProperty("eur")
    public float eur;
    @JsonProperty("gbp")
    public float gbp;
    @JsonProperty("hkd")
    public float hkd;
    @JsonProperty("huf")
    public float huf;
    @JsonProperty("idr")
    public float idr;
    @JsonProperty("ils")
    public float ils;
    @JsonProperty("inr")
    public float inr;
    @JsonProperty("jpy")
    public float jpy;
    @JsonProperty("krw")
    public float krw;
    @JsonProperty("kwd")
    public float kwd;
    @JsonProperty("lkr")
    public float lkr;
    @JsonProperty("ltc")
    public float ltc;
    @JsonProperty("mmk")
    public float mmk;
    @JsonProperty("mxn")
    public float mxn;
    @JsonProperty("myr")
    public float myr;
    @JsonProperty("ngn")
    public float ngn;
    @JsonProperty("nok")
    public float nok;
    @JsonProperty("nzd")
    public float nzd;
    @JsonProperty("php")
    public float php;
    @JsonProperty("pkr")
    public float pkr;
    @JsonProperty("pln")
    public float pln;
    @JsonProperty("rub")
    public float rub;
    @JsonProperty("sar")
    public float sar;
    @JsonProperty("sek")
    public float sek;
    @JsonProperty("sgd")
    public float sgd;
    @JsonProperty("thb")
    public float thb;
    @JsonProperty("try")
    public float _try;
    @JsonProperty("twd")
    public float twd;
    @JsonProperty("uah")
    public float uah;
    @JsonProperty("usd")
    public float usd;
    @JsonProperty("vef")
    public float vef;
    @JsonProperty("vnd")
    public float vnd;
    @JsonProperty("xag")
    public float xag;
    @JsonProperty("xau")
    public float xau;
    @JsonProperty("xdr")
    public float xdr;
    @JsonProperty("xlm")
    public float xlm;
    @JsonProperty("xrp")
    public float xrp;
    @JsonProperty("yfi")
    public float yfi;
    @JsonProperty("zar")
    public float zar;
    @JsonProperty("bits")
    public float bits;
    @JsonProperty("link")
    public float link;
    @JsonProperty("sats")
    public float sats;

}

@JsonInclude(JsonInclude.Include.NON_NULL)
class PriceChangePercentage60dInCurrency {

    @JsonProperty("aed")
    public float aed;
    @JsonProperty("ars")
    public float ars;
    @JsonProperty("aud")
    public float aud;
    @JsonProperty("bch")
    public float bch;
    @JsonProperty("bdt")
    public float bdt;
    @JsonProperty("bhd")
    public float bhd;
    @JsonProperty("bmd")
    public float bmd;
    @JsonProperty("bnb")
    public float bnb;
    @JsonProperty("brl")
    public float brl;
    @JsonProperty("btc")
    public float btc;
    @JsonProperty("cad")
    public float cad;
    @JsonProperty("chf")
    public float chf;
    @JsonProperty("clp")
    public float clp;
    @JsonProperty("cny")
    public float cny;
    @JsonProperty("czk")
    public float czk;
    @JsonProperty("dkk")
    public float dkk;
    @JsonProperty("dot")
    public float dot;
    @JsonProperty("eos")
    public float eos;
    @JsonProperty("eth")
    public float eth;
    @JsonProperty("eur")
    public float eur;
    @JsonProperty("gbp")
    public float gbp;
    @JsonProperty("hkd")
    public float hkd;
    @JsonProperty("huf")
    public float huf;
    @JsonProperty("idr")
    public float idr;
    @JsonProperty("ils")
    public float ils;
    @JsonProperty("inr")
    public float inr;
    @JsonProperty("jpy")
    public float jpy;
    @JsonProperty("krw")
    public float krw;
    @JsonProperty("kwd")
    public float kwd;
    @JsonProperty("lkr")
    public float lkr;
    @JsonProperty("ltc")
    public float ltc;
    @JsonProperty("mmk")
    public float mmk;
    @JsonProperty("mxn")
    public float mxn;
    @JsonProperty("myr")
    public float myr;
    @JsonProperty("ngn")
    public float ngn;
    @JsonProperty("nok")
    public float nok;
    @JsonProperty("nzd")
    public float nzd;
    @JsonProperty("php")
    public float php;
    @JsonProperty("pkr")
    public float pkr;
    @JsonProperty("pln")
    public float pln;
    @JsonProperty("rub")
    public float rub;
    @JsonProperty("sar")
    public float sar;
    @JsonProperty("sek")
    public float sek;
    @JsonProperty("sgd")
    public float sgd;
    @JsonProperty("thb")
    public float thb;
    @JsonProperty("try")
    public float _try;
    @JsonProperty("twd")
    public float twd;
    @JsonProperty("uah")
    public float uah;
    @JsonProperty("usd")
    public float usd;
    @JsonProperty("vef")
    public float vef;
    @JsonProperty("vnd")
    public float vnd;
    @JsonProperty("xag")
    public float xag;
    @JsonProperty("xau")
    public float xau;
    @JsonProperty("xdr")
    public float xdr;
    @JsonProperty("xlm")
    public float xlm;
    @JsonProperty("xrp")
    public float xrp;
    @JsonProperty("yfi")
    public float yfi;
    @JsonProperty("zar")
    public float zar;
    @JsonProperty("bits")
    public float bits;
    @JsonProperty("link")
    public float link;
    @JsonProperty("sats")
    public float sats;

}

@JsonInclude(JsonInclude.Include.NON_NULL)
class PriceChangePercentage7dInCurrency {

    @JsonProperty("aed")
    public float aed;
    @JsonProperty("ars")
    public float ars;
    @JsonProperty("aud")
    public float aud;
    @JsonProperty("bch")
    public float bch;
    @JsonProperty("bdt")
    public float bdt;
    @JsonProperty("bhd")
    public float bhd;
    @JsonProperty("bmd")
    public float bmd;
    @JsonProperty("bnb")
    public float bnb;
    @JsonProperty("brl")
    public float brl;
    @JsonProperty("btc")
    public float btc;
    @JsonProperty("cad")
    public float cad;
    @JsonProperty("chf")
    public float chf;
    @JsonProperty("clp")
    public float clp;
    @JsonProperty("cny")
    public float cny;
    @JsonProperty("czk")
    public float czk;
    @JsonProperty("dkk")
    public float dkk;
    @JsonProperty("dot")
    public float dot;
    @JsonProperty("eos")
    public float eos;
    @JsonProperty("eth")
    public float eth;
    @JsonProperty("eur")
    public float eur;
    @JsonProperty("gbp")
    public float gbp;
    @JsonProperty("hkd")
    public float hkd;
    @JsonProperty("huf")
    public float huf;
    @JsonProperty("idr")
    public float idr;
    @JsonProperty("ils")
    public float ils;
    @JsonProperty("inr")
    public float inr;
    @JsonProperty("jpy")
    public float jpy;
    @JsonProperty("krw")
    public float krw;
    @JsonProperty("kwd")
    public float kwd;
    @JsonProperty("lkr")
    public float lkr;
    @JsonProperty("ltc")
    public float ltc;
    @JsonProperty("mmk")
    public float mmk;
    @JsonProperty("mxn")
    public float mxn;
    @JsonProperty("myr")
    public float myr;
    @JsonProperty("ngn")
    public float ngn;
    @JsonProperty("nok")
    public float nok;
    @JsonProperty("nzd")
    public float nzd;
    @JsonProperty("php")
    public float php;
    @JsonProperty("pkr")
    public float pkr;
    @JsonProperty("pln")
    public float pln;
    @JsonProperty("rub")
    public float rub;
    @JsonProperty("sar")
    public float sar;
    @JsonProperty("sek")
    public float sek;
    @JsonProperty("sgd")
    public float sgd;
    @JsonProperty("thb")
    public float thb;
    @JsonProperty("try")
    public float _try;
    @JsonProperty("twd")
    public float twd;
    @JsonProperty("uah")
    public float uah;
    @JsonProperty("usd")
    public float usd;
    @JsonProperty("vef")
    public float vef;
    @JsonProperty("vnd")
    public float vnd;
    @JsonProperty("xag")
    public float xag;
    @JsonProperty("xau")
    public float xau;
    @JsonProperty("xdr")
    public float xdr;
    @JsonProperty("xlm")
    public float xlm;
    @JsonProperty("xrp")
    public float xrp;
    @JsonProperty("yfi")
    public float yfi;
    @JsonProperty("zar")
    public float zar;
    @JsonProperty("bits")
    public float bits;
    @JsonProperty("link")
    public float link;
    @JsonProperty("sats")
    public float sats;

}

@JsonInclude(JsonInclude.Include.NON_NULL)
class PublicInterestStats {

    @JsonProperty("alexa_rank")
    public Object alexaRank;
    @JsonProperty("bing_matches")
    public Object bingMatches;

}

@JsonInclude(JsonInclude.Include.NON_NULL)
class ReposUrl {

    @JsonProperty("github")
    public List<Object> github = null;
    @JsonProperty("bitbucket")
    public List<Object> bitbucket = null;

}


@JsonInclude(JsonInclude.Include.NON_NULL)
class Ticker {

    @JsonProperty("base")
    public String base;
    @JsonProperty("target")
    public String target;
    @JsonProperty("market")
    public Market market;
    @JsonProperty("last")
    public float last;
    @JsonProperty("volume")
    public float volume;
    @JsonProperty("converted_last")
    public ConvertedLast convertedLast;
    @JsonProperty("converted_volume")
    public ConvertedVolume convertedVolume;
    @JsonProperty("trust_score")
    public String trustScore;
    @JsonProperty("bid_ask_spread_percentage")
    public float bidAskSpreadPercentage;
    @JsonProperty("timestamp")
    public String timestamp;
    @JsonProperty("last_traded_at")
    public String lastTradedAt;
    @JsonProperty("last_fetch_at")
    public String lastFetchAt;
    @JsonProperty("is_anomaly")
    public boolean isAnomaly;
    @JsonProperty("is_stale")
    public boolean isStale;
    @JsonProperty("trade_url")
    public String tradeUrl;
    @JsonProperty("token_info_url")
    public Object tokenInfoUrl;
    @JsonProperty("coin_id")
    public String coinId;
    @JsonProperty("target_coin_id")
    public String targetCoinId;

}

@JsonInclude(JsonInclude.Include.NON_NULL)
class TotalVolume {

    @JsonProperty("aed")
    public float aed;
    @JsonProperty("ars")
    public float ars;
    @JsonProperty("aud")
    public float aud;
    @JsonProperty("bch")
    public float bch;
    @JsonProperty("bdt")
    public float bdt;
    @JsonProperty("bhd")
    public float bhd;
    @JsonProperty("bmd")
    public float bmd;
    @JsonProperty("bnb")
    public float bnb;
    @JsonProperty("brl")
    public float brl;
    @JsonProperty("btc")
    public float btc;
    @JsonProperty("cad")
    public float cad;
    @JsonProperty("chf")
    public float chf;
    @JsonProperty("clp")
    public float clp;
    @JsonProperty("cny")
    public float cny;
    @JsonProperty("czk")
    public float czk;
    @JsonProperty("dkk")
    public float dkk;
    @JsonProperty("dot")
    public float dot;
    @JsonProperty("eos")
    public float eos;
    @JsonProperty("eth")
    public float eth;
    @JsonProperty("eur")
    public float eur;
    @JsonProperty("gbp")
    public float gbp;
    @JsonProperty("hkd")
    public float hkd;
    @JsonProperty("huf")
    public float huf;
    @JsonProperty("idr")
    public long idr;
    @JsonProperty("ils")
    public float ils;
    @JsonProperty("inr")
    public float inr;
    @JsonProperty("jpy")
    public float jpy;
    @JsonProperty("krw")
    public float krw;
    @JsonProperty("kwd")
    public float kwd;
    @JsonProperty("lkr")
    public float lkr;
    @JsonProperty("ltc")
    public float ltc;
    @JsonProperty("mmk")
    public float mmk;
    @JsonProperty("mxn")
    public float mxn;
    @JsonProperty("myr")
    public float myr;
    @JsonProperty("ngn")
    public float ngn;
    @JsonProperty("nok")
    public float nok;
    @JsonProperty("nzd")
    public float nzd;
    @JsonProperty("php")
    public float php;
    @JsonProperty("pkr")
    public float pkr;
    @JsonProperty("pln")
    public float pln;
    @JsonProperty("rub")
    public float rub;
    @JsonProperty("sar")
    public float sar;
    @JsonProperty("sek")
    public float sek;
    @JsonProperty("sgd")
    public float sgd;
    @JsonProperty("thb")
    public float thb;
    @JsonProperty("try")
    public float _try;
    @JsonProperty("twd")
    public float twd;
    @JsonProperty("uah")
    public float uah;
    @JsonProperty("usd")
    public float usd;
    @JsonProperty("vef")
    public float vef;
    @JsonProperty("vnd")
    public long vnd;
    @JsonProperty("xag")
    public float xag;
    @JsonProperty("xau")
    public float xau;
    @JsonProperty("xdr")
    public float xdr;
    @JsonProperty("xlm")
    public float xlm;
    @JsonProperty("xrp")
    public float xrp;
    @JsonProperty("yfi")
    public float yfi;
    @JsonProperty("zar")
    public float zar;
    @JsonProperty("bits")
    public float bits;
    @JsonProperty("link")
    public float link;
    @JsonProperty("sats")
    public float sats;
}